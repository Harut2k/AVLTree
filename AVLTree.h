...
protected:
	Node * HeadPtr; // head of tree
	Node **Array; // array representation of tree
...
protected:
	...
	int generateArray(Node **&arr);

	// adds all children to given array
	static int addToArray(Node *n_ptr, Node **&arr, int i);
...
public:
	...
	T & operator[](const unsigned i) { return Array[i]->key; }
	T operator[](const unsigned i) const { return Array[i]->key; }
...
template<typename T>
int SearchTree<T>::addToArray(Node *n_ptr, Node **&arr, int i) {
	if (n_ptr == nullptr)
		return i;

	if (n_ptr->left != nullptr)
		i = addToArray(n_ptr->left, arr, i);
	...
	if (n_ptr->right != nullptr)
		i = addToArray(n_ptr->right, arr, i);
...
inline SearchTree<T>::SearchTree()
	: size_(0), HeadPtr(nullptr), Array(nullptr), ArrayValid(false) {
}
...
template<typename T>
inline int SearchTree<T>::generateArray(Node **&arr) {
	int size = numChildren_(HeadPtr);
	if (arr != nullptr)
		delete[] arr;
	arr = new Node*[size];
	addToArray(HeadPtr, arr, 0);
	return size;
}
...
bool SearchTree<T>::remove(const T key) {
	if (HeadPtr != nullptr) {
		Node *n_ptr = HeadPtr, *parent = HeadPtr, *child = HeadPtr;
...
inline int SearchTree<T>::find(const T key) const {
	return (HeadPtr == nullptr) ? -1 : find_(key, HeadPtr);
}
